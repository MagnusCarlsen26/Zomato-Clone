...
const Restaurants = () =>{
        ...
        return (
            <div className='cuisine-sort' >
                ...
                <div className='searches'>
                    <svg className='svg-cuisine' xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                ...
                <div style={{display:'flex',justifyContent:'space-between'}}>
                    <span onClick={() => {setCheckedCuisine( () => new Array(cuisineType.length).fill(false))}}>clear all</span>
                    <span onClick={() => {setCuisineHandler( (item) => {console.log(apply);return !item;} ) ;setApply((apply)=>(apply+1)) }}> apply </span>
                </div>
            </div>
    ...
    useEffect( () => {
        var c = 0
        const t = checkedCuisine.map( (item,index) => { if (item)  { c=c+1;return(<div className='filter' key={index}>{cuisineType[index]}</div>)}; } )
        setExtra(t)
        setNumber(c)
    },[apply])    
        ...
        <>
            <div className='sticky-filters'>
                <div className='filters'  style={{marginRight:200,marginLeft:200,marginTop:10,marginBottom:20,paddingTop:20}}>
                    ...
                    {extra}
                    <div className='filter' onClick={() => setSortRating( (item) => !item )}>Rating 4.0+</div>
                    <div style={{position:'relative'}} >
                        <div className='filter' onClick={() => {setCuisineHandler(()=>!cuisineHandler) }}>
                            <p>Cuisines</p>
